...
uint8_t ADC_ConvertCount;           //AD转换次数，1ms自增1
uint8_t TxBuff[11];
...
uint8_t Send_CAN_DataFrame_Single(uint8_t *ptr, uint8_t len) //ptr: 数据指针. len: 数据长度 
{
	...
	while((CAN_TransmitStatus(CAN1, mailbox_num) != CANTXOK)&&(i<500)) //发送状态,等到发送完成  
	{
		i++;
	}
	if(i==500)
...
uint16_t Get_Adc(void)   
{
	...
	//设置指定ADC的规则组通道，一个序列，采样时间
	ADC_RegularChannelConfig(ADC1,ADC_Channel_14, 1, ADC_SampleTime_55Cycles5 );	//ADC1,ADC通道,采样时间为239.5周期	  			    
	...
	//设置指定ADC的规则组通道，一个序列，采样时间
	ADC_RegularChannelConfig(ADC1,ADC_Channel_15, 1, ADC_SampleTime_55Cycles5 );	//ADC1,ADC通道,采样时间为239.5周期	  			    
		...
		if(!CPUofflineFlag)//控制命令在线
		{
			...
			if((Convert_num==3)&&(!CPUofflineFlag))
			{
				...
				if(SignalSetFlag)    //判断外灯是否处于闪灯状态
				{
					...
					CurrentValue1 = Average_Current1temp;//(uint8_t)((Average_Current1[0]+Average_Current1[1]+Average_Current1[2])/3);
					CurrentValue2 = Average_Current2temp;//(uint8_t)((Average_Current2[0]+Average_Current2[1]+Average_Current2[2])/3);
					CurrentValue3 = Average_Current3temp;//(uint8_t)((Average_Current3[0]+Average_Current3[1]+Average_Current3[2])/3);
					CurrentValue4 = Average_Current4temp;//(uint8_t)((Average_Current4[0]+Average_Current4[1]+Average_Current4[2])/3);
